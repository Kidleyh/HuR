...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tracker-config", default="configs/bytetrack_person.yaml")
...
def _is_complete(output_dir: Path, visualization: bool) -> bool:
    required = ["detections.jsonl", "detections.csv", "tracks_summary.json"]
    if visualization:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        for video in videos:
            try:
                _record_error(video, output_root / video.stem, error)
    ...
    for index, video in enumerate(videos, start=1):
        video_output = output_root / video.stem
        if _is_complete(video_output, args.save_visualization) and not args.overwrite:
            skipped += 1
        ...
        try:
            result = tracker.track_video(
                str(video), str(video_output), save_visualization=args.save_visualization
            )
            succeeded += 1
            LOGGER.info(
                "Completed %s: %d frames, %d tracks, %.2fs (%.2f FPS)",
                video.name,
